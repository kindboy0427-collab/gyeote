...
const plans: PaymentPlan[] = [
  {
    id: 'monthly',
    name: '월간 구독',
    description: '매월 자동 결제 · 언제든지 해지 가능',
    price: 4900,
    originalPrice: 5900,
    discountRate: 17,
    cycleText: '월',
  ...
  return (
    <main className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-10">
      <div className="bg-white rounded-2xl shadow-sm p-8 max-w-md w-full">
        <h1 className="text-2xl font-bold text-green-600 mb-2 text-center">곁에 구독</h1>
        <p className="text-gray-500 text-sm text-center mb-8">
          부모님 안부 확인을 위한 구독 플랜을 선택해주세요.
        </p>

        <section className="mb-6">
          <h2 className="text-sm font-bold text-gray-800 mb-3">1. 구독 플랜 선택</h2>
...
{plan.id === 'yearly' && (
  <p className="text-xs text-green-600 mt-1">하루 136원</p>
        ...
        <section className="mb-6">
          <h2 className="text-sm font-bold text-gray-800 mb-3">2. 결제수단 선택</h2>
        ...
        <div className="bg-gray-50 rounded-xl p-5 mb-6">
          <p className="text-sm font-bold text-gray-800 mb-2">선택한 구독</p>
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-600">
              {selectedPlan.name} · {selectedPlan.cycleText}마다 자동 결제
            </p>
            <div className="text-right">
              ...
              <p className="text-sm font-bold text-green-600">{formatPrice(selectedPlan.price)}원</p>
            </div>
          </div>
          <p className="text-sm text-gray-600 mt-1">결제수단: {selectedProvider.name}</p>
        </div>
        ...
        >
          {loading ? '구독 결제 준비 중...' : `${selectedPlan.name} 시작하기`}
        </button>

        <p className="text-xs text-gray-400 text-center mt-4">
          구독은 직접 해지하기 전까지 {selectedPlan.cycleText}마다 자동 결제됩니다.
        </p>
      </div>
    </main>
  )
